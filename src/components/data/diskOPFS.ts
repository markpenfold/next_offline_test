...
/**
 * Scans local OPFS data shard files in /shards directory
 */
/**
 * Scans local OPFS data shard files stored in the /data directory
 * (where syncFullDataShards saves them).
 */
export async function getLocalOPFSDataShards(
  onLog?: (msg: string) => void
): Promise<AvailableDataShard[]> {
  const log = (msg: string) => onLog?.(msg);
  ...
  try {
    ...
    for (const { name, handle } of entries) {
      if (name.endsWith('.parquet')) {
        ...
        // e.g., "shard__free__finance__2024__v1.parquet"
    ...
    log(`✅ Discovered ${foundShards.length} parquet shard file(s) in OPFS (/data).`);
    return foundShards;
  } catch (err: any) {
    log(`❌ Error scanning OPFS /data directory: ${err.message}`);
    console.error(err);
    return [];
  }
}
